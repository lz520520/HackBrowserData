...
func (c *ChromiumPassword) Extract(masterKey []byte) error {
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&url, &username, &pwd, &create); err != nil {
            logger.Error("scan chromium password error", "err", err)
        }
        login := loginData{
            UserName:    username,
            encryptPass: pwd,
        ...
        if len(pwd) > 0 {
            ...
            if err != nil {
                logger.Error("decrypt chromium password error", "err", err)
            }
        }
...
func (c *YandexPassword) Extract(masterKey []byte) error {
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&url, &username, &pwd, &create); err != nil {
            logger.Error("scan yandex password error", "err", err)
        }
        login := loginData{
            UserName:    username,
            encryptPass: pwd,
        ...
        if len(pwd) > 0 {
            ...
            if err != nil {
                logger.Error("decrypt yandex password error", "err", err)
            }
        }
        if create > time.Now().Unix() {
            login.CreateDate = typeutil.TimeEpoch(create)
